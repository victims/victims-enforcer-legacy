...
/**
 * The Database class provides a layer between the JSON 
 * content retrieved from the master database and the the local SQL database
 * in which fingerprints are cached.
 * @author gmurphy
 */
public class Database {
    ...
    /**
     * Create a new database instance. The database content will be stored 
    ...
    /**
     * Create a new database instance. The database content will be stored 
     * at the specified location. All errors will be reported on the supplied 
     * logging mechanism.
     * @param db
     * @param l 
     */
     ...
     * enumeration. Values from the supplied JSON object are then mapped to 
     * an SQL query as necessary. 
     * 
     * @param id - The operation to perform.
     * @param json - The object to use when performing the operation. 
     * @return - A JSONObject that contains a true/false "result" indicator 
     * and also any items that it has found in either a 'collection' entry where 
    ...
    /**
     * Returns a operation that will insert a JSON object into the database 
     * when executed. 
     * 
    ...
    /**
     * Returns an operation that will remove any objects from the database 
     * that match the supplied primary key (SHA-512).
     * 
    ...
    /**
     * Returns a statement that when executed returns the latest version 
     * of the Victims database that we have locally. This is used when 
     * synchronizing to the master database. 
     * 
     ...
     * If an existing JSONObject is also supplied it has all of the keys / values
     * associated with it copied to the returned value. 
     * 
    ...
    private JSONObject createResultJSON(boolean result, JSONObject obj) throws JSONException {
      ...
      /**
     * Converts a SQL result set into a JSON object. Only the specified 
     * keys will be populated within the JSON object. 
    ...
    /**
     * Creates a connection to an embedded Apache Derby instance. If the database
     ...
     * @return The white list of keys that are allowed to be added to 
     * a JSON object. 
     */
     ...
     * @return The corresponding statement that can be executed in a transaction.
     * @throws VictimsException 
     */
    private Statement resolveStatement(Statements id) throws VictimsException {
        ...
        switch(id){
            ...
            case CHECK_HASH : return containsHash();
            case CHECK_JAR  : return containsJAR();
